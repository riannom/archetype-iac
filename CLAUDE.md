...
Netlab GUI is a CML-like web interface for netlab/containerlab. It provides a drag-and-drop topology canvas, YAML import/export, lab lifecycle management (up/down/restart), and WebSocket-based node console access.
...
- **Job queue**: Redis + RQ (`api/app/jobs.py`) - async execution of `netlab up/down/restart`
- **Topology**: `api/app/topology.py` - converts between GUI graph JSON and netlab YAML
...
2. `POST /labs/{id}/up` → enqueues RQ job → worker runs `netlab up` in lab workspace
3. Console: WebSocket at `/labs/{id}/nodes/{node}/console` → spawns `netlab connect {node}`

### Key Patterns
- Lab workspaces stored at `NETLAB_WORKSPACE` (default `/var/lib/netlab-gui/{lab_id}/`)
- Each lab has a `topology.yml` file that netlab reads
- Worker runs with `network_mode: host` and `privileged: true` to access containerlab/docker
...
- `NETLAB_WORKSPACE`: Root directory for lab files
- `NETLAB_PROVIDER`: `clab` (containerlab) is the primary supported provider
