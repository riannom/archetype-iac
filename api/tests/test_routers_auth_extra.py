...
def test_register_duplicate_and_password_length(test_client, test_db, monkeypatch) -> None:
    ...
    resp = test_client.post("/auth/register", json={"email": "dup@example.com", "password": "pass"})
...
def test_me_and_preferences(test_client, test_db, test_user, auth_headers) -> None:
    ...
    update = {
        ...
        "canvas_settings": {"grid": {"enabled": True}},
    }
    ...
    assert body["canvas_settings"]["grid"]["enabled"] is True
