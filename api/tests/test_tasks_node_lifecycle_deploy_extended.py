...
class TestCreateAndStartNodes:
    ...
    async def test_desired_state_filter_skips_changed_nodes(self, test_db, test_user):
        ...
        ns = make_node_state(
            test_db, lab.id, "n1", "R1", desired_state="stopped", actual_state="undeployed"
        )
    ...
    async def test_non_ceos_deploys_in_parallel(self, test_db, test_user):
        ...
        node_def1 = make_node(test_db, lab.id, gui_id="n1", display_name="R1", container_name="R1", device="linux", host_id=host.id)
        node_def2 = make_node(test_db, lab.id, gui_id="n2", display_name="R2", container_name="R2", device="linux", host_id=host.id)
    ...
    async def test_mixed_ceos_and_non_ceos(self, test_db, test_user):
        ...
        host, lab, job = self._base_manager(test_db, test_user)
        ns_linux = make_node_state(test_db, lab.id, "n1", "R1", desired_state="running", actual_state="undeployed")
        ns_ceos = make_node_state(test_db, lab.id, "n2", "R2", desired_state="running", actual_state="undeployed")
        node_def_linux = make_node(test_db, lab.id, gui_id="n1", display_name="R1", container_name="R1", device="linux", host_id=host.id)
        node_def_ceos = make_node(test_db, lab.id, gui_id="n2", display_name="R2", container_name="R2", device="arista_ceos", host_id=host.id)
    ...
    async def test_mixed_provider_partial_success(self, test_db, test_user):
        ...
        host, lab, job = self._base_manager(test_db, test_user)
        ns_docker = make_node_state(test_db, lab.id, "n1", "R1", desired_state="running", actual_state="undeployed")
        ns_libvirt = make_node_state(test_db, lab.id, "n2", "XRV1", desired_state="running", actual_state="undeployed")
        node_def_docker = make_node(test_db, lab.id, gui_id="n1", display_name="R1", container_name="R1", device="linux", host_id=host.id)
        node_def_libvirt = make_node(test_db, lab.id, gui_id="n2", display_name="XRV1", container_name="XRV1", device="cisco_iosxr", host_id=host.id)
...
class TestStartNodesTopology:
    ...
    def _setup(self, test_db, test_user):
        ...
        job = make_job(test_db, lab.id, test_user.id)
        ns = make_node_state(
            test_db, lab.id, "n1", "R1", desired_state="running", actual_state="stopped"
        )
...
class TestConnectSameHostLinksExtended:
    ...
    async def test_no_eligible_links_is_noop(self, test_db, test_user):
        ...
        ns1 = make_node_state(test_db, lab.id, "n1", "R1", actual_state="running")
    ...
    async def test_link_orchestration_exception_is_logged_not_raised(self, test_db, test_user):
        ...
        ns1 = make_node_state(test_db, lab.id, "n1", "R1", actual_state="running")
        ns2 = make_node_state(test_db, lab.id, "n2", "R2", actual_state="running")
        ns1.is_ready = True
        ns2.is_ready = True
        test_db.commit()
        make_placement(test_db, lab.id,"R1", host.id)
    ...
    async def test_first_link_fails_second_still_attempted(self, test_db, test_user):
        ...
        ns1 = make_node_state(test_db, lab.id, "n1", "R1", actual_state="running")
        ns2 = make_node_state(test_db, lab.id, "n2", "R2", actual_state="running")
        ns3 = make_node_state(test_db, lab.id, "n3", "R3", actual_state="running")
        ns1.is_ready = True
        ns2.is_ready = True
        ns3.is_ready = True
        test_db.commit()
