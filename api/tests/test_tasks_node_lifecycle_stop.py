...
class TestStopNodesSequencing:
    ...
    async def test_stop_multiple_nodes_same_agent(self, test_db, test_user):
        ...
        lab = make_lab(test_db, test_user.id, state="running", agent_id=host.id)
        job = make_job(test_db, lab.id, test_user.id)
        ns1 = make_node_state(test_db, lab.id, "n1", "R1", desired_state="stopped", actual_state="running")
        ns2 = make_node_state(test_db, lab.id, "n2", "R2", desired_state="stopped", actual_state="running")
...
class TestStopAgentFailures:
    ...
    async def test_fallback_to_default_agent_on_not_found(self, test_db, test_user):
        ...
        remote_host = make_host(test_db, host_id="remote-host", name="Remote Host")
        lab = make_lab(test_db, test_user.id, state="running", agent_id=main_host.id)
        job = make_job(test_db, lab.id, test_user.id)
        ns = make_node_state(test_db, lab.id, "n1", "R1", desired_state="stopped", actual_state="running")
        make_placement(test_db, lab.id, "R1", remote_host.id)
        container_name = _get_container_name(lab.id, "R1")
...
class TestStopMultiAgentGrouping:
    ...
    async def test_nodes_grouped_by_placement_agent(self, test_db, test_user):
        ...
        ns1 = make_node_state(test_db, lab.id, "n1", "R1", desired_state="stopped", actual_state="running")
        ns2 = make_node_state(test_db, lab.id, "n2", "R2", desired_state="stopped", actual_state="running")
        make_placement(test_db, lab.id, "R1", host_a.id)
        make_placement(test_db, lab.id, "R2", host_b.id)
    ...
    async def test_offline_placement_agent_falls_back_to_default(self, test_db, test_user):
        ...
        main_host = make_host(test_db, host_id="main-host", name="Main Host")
        offline_host = make_host(test_db, host_id="offline-host", name="Offline Host", status="offline")
        lab = make_lab(test_db, test_user.id, state="running", agent_id=main_host.id)
        job = make_job(test_db, lab.id, test_user.id)
        ns = make_node_state(test_db, lab.id, "n1", "R1", desired_state="stopped", actual_state="running")
        make_placement(test_db, lab.id, "R1", offline_host.id)
        container_name = _get_container_name(lab.id, "R1")
...
class TestAutoExtractBeforeStop:
    ...
    async def test_successful_extraction_creates_snapshots(self, test_db, test_user):
        ...
        job = make_job(test_db, lab.id, test_user.id)
        ns = make_node_state(test_db, lab.id, "n1", "R1", desired_state="stopped", actual_state="running")
        make_node(test_db, lab.id, gui_id="n1", display_name="R1", container_name="R1", device="linux", host_id=host.id)
...
class TestConvergeStoppedDesiredErrorStates:
    ...
    def test_multiple_error_nodes_all_normalized(self, test_db, test_user):
        ...
        lab = make_lab(test_db, test_user.id, state="running", agent_id=host.id)
        job = make_job(test_db, lab.id, test_user.id)
        ns1 = make_node_state(test_db, lab.id, "n1", "R1", desired_state="stopped", actual_state="error")
        ns2 = make_node_state(test_db, lab.id, "n2", "R2", desired_state="stopped", actual_state="error")
