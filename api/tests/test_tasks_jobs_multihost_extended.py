...
class TestMultihostDeployResourceCapacity:
    ...
    async def test_resource_capacity_failure_fails_job(self, test_db: Session, test_user):
        ...
        host = make_host(test_db, host_id="host-cap-fail")
        make_node(test_db, lab.id, gui_id="r1", display_name="R1", host_id=host.id)
    ...
    async def test_resource_capacity_warnings_logged(self, test_db: Session, test_user):
        ...
        lab = make_lab(test_db, test_user.id)
        job = make_job(test_db, lab.id, test_user.id, action="up")
        host = make_host(test_db, host_id="host-cap-warn")
        make_node(test_db, lab.id, gui_id="r1", display_name="R1", host_id=host.id)
...
class TestMultihostDeployPreflightFailure:
    ...
    async def test_preflight_failure_marks_host_missing(self, test_db: Session, test_user):
        ...
        lab = make_lab(test_db, test_user.id)
        job = make_job(test_db, lab.id, test_user.id, action="up")
        host = make_host(test_db, host_id="host-preflight-fail")
        make_node(test_db, lab.id, gui_id="r1", display_name="R1", host_id=host.id)
    ...
    async def test_offline_agent_marks_host_missing(self, test_db: Session, test_user):
        ...
        lab = make_lab(test_db, test_user.id)
        job = make_job(test_db, lab.id, test_user.id, action="up")
        host = make_host(test_db, host_id="host-offline-deploy", status="offline")
        make_node(test_db, lab.id, gui_id="r1", display_name="R1", host_id=host.id)
...
class TestMultihostDeployLinkFailure:
    ...
    async def test_link_failure_fails_job(self, test_db: Session, test_user):
        ...
        lab = make_lab(test_db, test_user.id)
        job = make_job(test_db, lab.id, test_user.id, action="up")
        host = make_host(test_db, host_id="host-link-fail")
        make_node(test_db, lab.id, gui_id="r1", display_name="R1", host_id=host.id)
    ...
    async def test_zero_link_failures_succeeds(self, test_db: Session, test_user):
        ...
        lab = make_lab(test_db, test_user.id)
        job = make_job(test_db, lab.id, test_user.id, action="up")
        host = make_host(test_db, host_id="host-link-ok")
        make_node(test_db, lab.id, gui_id="r1", display_name="R1", host_id=host.id)
...
class TestMultihostDeployHappyPath:
    ...
    async def test_successful_deploy_sets_running_state(self, test_db: Session, test_user):
        ...
        lab = make_lab(test_db, test_user.id)
        job = make_job(test_db, lab.id, test_user.id, action="up")
        host = make_host(test_db, host_id="host-happy")
        make_node(test_db, lab.id, gui_id="r1", display_name="R1", host_id=host.id)
    ...
    async def test_deploy_broadcasts_started_and_completed(self, test_db: Session, test_user):
        ...
        lab = make_lab(test_db, test_user.id)
        job = make_job(test_db, lab.id, test_user.id, action="up")
        host = make_host(test_db, host_id="host-broadcast")
        make_node(test_db, lab.id, gui_id="r1", display_name="R1", host_id=host.id)
    ...
    async def test_first_agent_set_as_primary(self, test_db: Session, test_user):
        ...
        lab = make_lab(test_db, test_user.id)
        job = make_job(test_db, lab.id, test_user.id, action="up")
        host_a = make_host(test_db, host_id="host-primary-a", name="Primary A")
        host_b = make_host(test_db, host_id="host-primary-b", name="Primary B")
        make_node(test_db, lab.id, gui_id="r1", display_name="R1", host_id=host_a.id)
        make_node(test_db, lab.id, gui_id="r2", display_name="R2", host_id=host_b.id)
...
class TestMultihostDeployUnplacedNoAgent:
    ...
    async def test_no_default_agent_fails_job(self, test_db: Session, test_user):
        ...
        lab = make_lab(test_db, test_user.id)
        job = make_job(test_db, lab.id, test_user.id, action="up")
        make_node(test_db, lab.id, gui_id="r1", display_name="R1")  # No host_id
...
class TestMultihostDeployRollbackFailure:
    ...
    async def test_rollback_exception_logged(self, test_db: Session, test_user):
        ...
        lab = make_lab(test_db, test_user.id)
        job = make_job(test_db, lab.id, test_user.id, action="up")
        host_a = make_host(test_db, host_id="host-rb-a", name="HostA")
        host_b = make_host(test_db, host_id="host-rb-b", name="HostB")
        make_node(test_db, lab.id, gui_id="r1", display_name="R1", host_id=host_a.id)
        make_node(test_db, lab.id, gui_id="r2", display_name="R2", host_id=host_b.id)
    ...
    async def test_all_deploys_fail_no_rollback_needed(self, test_db: Session, test_user):
        ...
        lab = make_lab(test_db, test_user.id)
        job = make_job(test_db, lab.id, test_user.id, action="up")
        host = make_host(test_db, host_id="host-all-fail")
        make_node(test_db, lab.id, gui_id="r1", display_name="R1", host_id=host.id)
    ...
    async def test_mixed_provider_host_rolls_back_only_successful_provider(
        ...
        lab = make_lab(test_db, test_user.id)
        job = make_job(test_db, lab.id, test_user.id, action="up")
        host = make_host(test_db, host_id="host-mixed-rb", name="HostMixed")
        make_node(test_db, lab.id, gui_id="ceos_1", display_name="ceos_1", host_id=host.id, device="ceos")
        make_node(test_db, lab.id, gui_id="n9kv_1", display_name="n9kv_1", host_id=host.id, device="cisco_n9kv")
...
class TestMultihostDeployResultLogging:
    ...
    async def test_stdout_stderr_in_log(self, test_db: Session, test_user):
        ...
        lab = make_lab(test_db, test_user.id)
        job = make_job(test_db, lab.id, test_user.id, action="up")
        host = make_host(test_db, host_id="host-log", name="LogHost")
        make_node(test_db, lab.id, gui_id="r1", display_name="R1", host_id=host.id)
...
class TestMultihostDestroyHappyPath:
    ...
    async def test_successful_destroy_sets_stopped_state(self, test_db: Session, test_user):
        ...
        host = make_host(test_db, host_id="host-destroy-ok")
        make_node(test_db, lab.id, gui_id="r1", display_name="R1", host_id=host.id)
    ...
    async def test_link_states_deleted_on_full_success(self, test_db: Session, test_user):
        ...
        lab = make_lab(test_db, test_user.id, state="running")
        job = make_job(test_db, lab.id, test_user.id, action="down")
        host = make_host(test_db, host_id="host-destroy-clean")
        make_node(test_db, lab.id, gui_id="r1", display_name="R1", host_id=host.id)
    ...
    async def test_mixed_provider_host_destroys_each_provider(
        ...
        lab = make_lab(test_db, test_user.id, state="running")
        job = make_job(test_db, lab.id, test_user.id, action="down")
        host = make_host(test_db, host_id="host-mixed-destroy")
        make_node(test_db, lab.id, gui_id="ceos_1", display_name="ceos_1", host_id=host.id, device="ceos")
        make_node(test_db, lab.id, gui_id="n9kv_1", display_name="n9kv_1", host_id=host.id, device="cisco_n9kv")
...
class TestMultihostDeployMixedProviders:
    ...
    async def test_mixed_provider_host_deploys_each_provider(
        ...
        lab = make_lab(test_db, test_user.id)
        job = make_job(test_db, lab.id, test_user.id, action="up")
        host = make_host(test_db, host_id="host-mixed-deploy", name="HostMixed")
        make_node(test_db, lab.id, gui_id="ceos_1", display_name="ceos_1", host_id=host.id, device="ceos")
        make_node(test_db, lab.id, gui_id="n9kv_1", display_name="n9kv_1", host_id=host.id, device="cisco_n9kv")
...
class TestMultihostDestroyTunnelTeardown:
    ...
    async def test_tunnel_failures_cause_warnings(self, test_db: Session, test_user):
        ...
        lab = make_lab(test_db, test_user.id, state="running")
        job = make_job(test_db, lab.id, test_user.id, action="down")
        host = make_host(test_db, host_id="host-tunnel-fail")
        make_node(test_db, lab.id, gui_id="r1", display_name="R1", host_id=host.id)
...
class TestMultihostDestroyMixedAgents:
    ...
    async def test_some_online_some_missing(self, test_db: Session, test_user):
        ...
        lab = make_lab(test_db, test_user.id, state="running")
        job = make_job(test_db, lab.id, test_user.id, action="down")
        host = make_host(test_db, host_id="host-online-mix")
        make_node(test_db, lab.id, gui_id="r1", display_name="R1", host_id=host.id)
        make_node(test_db, lab.id, gui_id="r2", display_name="R2", host_id="nonexistent-host-id")
    ...
    async def test_some_online_some_offline(self, test_db: Session, test_user):
        ...
        lab = make_lab(test_db, test_user.id, state="running")
        job = make_job(test_db, lab.id, test_user.id, action="down")
        host_on = make_host(test_db, host_id="host-on-mixed", name="OnlineHost")
        host_off = make_host(test_db, host_id="host-off-mixed", name="OfflineHost", status="offline")
        make_node(test_db, lab.id, gui_id="r1", display_name="R1", host_id=host_on.id)
        make_node(test_db, lab.id, gui_id="r2", display_name="R2", host_id=host_off.id)
...
class TestMultihostDestroyResultLogging:
    ...
    async def test_stdout_stderr_truncated_in_log(self, test_db: Session, test_user):
        ...
        lab = make_lab(test_db, test_user.id, state="running")
        job = make_job(test_db, lab.id, test_user.id, action="down")
        host = make_host(test_db, host_id="host-trunc", name="TruncHost")
        make_node(test_db, lab.id, gui_id="r1", display_name="R1", host_id=host.id)
...
class TestMultihostDestroyWebhooks:
    ...
    async def test_partial_failure_dispatches_job_failed(self, test_db: Session, test_user):
        ...
        lab = make_lab(test_db, test_user.id, state="running")
        job = make_job(test_db, lab.id, test_user.id, action="down")
        host = make_host(test_db, host_id="host-wh-fail")
        make_node(test_db, lab.id, gui_id="r1", display_name="R1", host_id=host.id)
...
class TestMultihostDeployThreeHosts:
    ...
    async def test_three_hosts_one_fails_triggers_rollback_of_two(
        ...
        job = make_job(test_db, lab.id, test_user.id, action="up")
        hosts = [
            make_host(test_db, host_id=f"host-3h-{i}", name=f"Host{i}")
            for i in range(3)
        ]
        for i, h in enumerate(hosts):
            make_node(test_db, lab.id, gui_id=f"r{i+1}", display_name=f"R{i+1}", host_id=h.id)
...
class TestMultihostDeployJobTimestamps:
    ...
    async def test_job_transitions_to_running(self, test_db: Session, test_user):
        ...
        lab = make_lab(test_db, test_user.id)
        job = make_job(test_db, lab.id, test_user.id, action="up")
        host = make_host(test_db, host_id="host-ts")
        make_node(test_db, lab.id, gui_id="r1", display_name="R1", host_id=host.id)
...
class TestMultihostDestroyJobLifecycle:
    ...
    async def test_destroy_sets_stopping_state(self, test_db: Session, test_user):
        ...
        lab = make_lab(test_db, test_user.id, state="running")
        job = make_job(test_db, lab.id, test_user.id, action="down")
        host = make_host(test_db, host_id="host-lifecycle")
        make_node(test_db, lab.id, gui_id="r1", display_name="R1", host_id=host.id)
    ...
    async def test_destroy_record_started_called_with_down(self, test_db: Session, test_user):
        ...
        lab = make_lab(test_db, test_user.id, state="running")
        job = make_job(test_db, lab.id, test_user.id, action="down")
        host = make_host(test_db, host_id="host-rs-down")
        make_node(test_db, lab.id, gui_id="r1", display_name="R1", host_id=host.id)
...
class TestMultihostDeployWebhookStarted:
    ...
    async def test_deploy_started_webhook_dispatched(self, test_db: Session, test_user):
        ...
        lab = make_lab(test_db, test_user.id)
        job = make_job(test_db, lab.id, test_user.id, action="up")
        host = make_host(test_db, host_id="host-wh-start")
        make_node(test_db, lab.id, gui_id="r1", display_name="R1", host_id=host.id)
...
class TestMultihostDeployStartingState:
    ...
    async def test_lab_state_set_to_starting(self, test_db: Session, test_user):
        ...
        lab = make_lab(test_db, test_user.id)
        job = make_job(test_db, lab.id, test_user.id, action="up")
        host = make_host(test_db, host_id="host-starting")
        make_node(test_db, lab.id, gui_id="r1", display_name="R1", host_id=host.id)
