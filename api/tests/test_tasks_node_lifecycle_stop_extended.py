...
class TestAutoExtractStoppingState:
    ...
    async def test_stopping_nodes_are_extractable(self, test_db, test_user):
        ...
        host = make_host(test_db, host_id="agent-stop-1")
        lab = make_lab(test_db, test_user.id, name="StopTest Lab", workspace_path="/tmp/stop-test", agent_id=host.id)
        job = make_job(test_db, lab.id, test_user.id, status="running", action="sync:lab")

        make_node(test_db, lab.id, gui_id="r1", display_name="R1")
        ns = make_node_state(test_db, lab.id, "r1", "R1", actual_state="stopping")
...
class TestAutoExtractAgentFallback:
    ...
    async def test_offline_placement_agent_falls_back_to_default(self, test_db, test_user):
        ...
        host_default = make_host(test_db, host_id="agent-ext-def", name="Default")
        host_offline = make_host(test_db, host_id="agent-ext-off", name="Offline", status="offline")
        lab = make_lab(test_db, test_user.id, name="StopTest Lab", workspace_path="/tmp/stop-test", agent_id=host_default.id)
        job = make_job(test_db, lab.id, test_user.id, status="running", action="sync:lab")

        make_node(test_db, lab.id, gui_id="r1", display_name="R1")
        ns = make_node_state(test_db, lab.id, "r1", "R1", actual_state="running")
    ...
    async def test_partial_timeout_keeps_successful_agent_results(self, test_db, test_user):
        ...
        host_a = make_host(test_db, host_id="agent-ext-a", name="Agent A")
        host_b = make_host(test_db, host_id="agent-ext-b", name="Agent B")
        lab = make_lab(test_db, test_user.id, name="StopTest Lab", workspace_path="/tmp/stop-test", agent_id=host_a.id)
        job = make_job(test_db, lab.id, test_user.id, status="running", action="sync:lab")

        make_node(test_db, lab.id, gui_id="r1", display_name="R1")
        make_node(test_db, lab.id, gui_id="r2", display_name="R2")
        ns1 = make_node_state(test_db, lab.id, "r1", "R1", actual_state="running")
        ns2 = make_node_state(test_db, lab.id, "r2", "R2", actual_state="running")
    ...
    async def test_no_placement_defaults_to_self_agent(self, test_db, test_user):
        ...
        host = make_host(test_db, host_id="agent-ext-noplace")
        lab = make_lab(test_db, test_user.id, name="StopTest Lab", workspace_path="/tmp/stop-test", agent_id=host.id)
        job = make_job(test_db, lab.id, test_user.id, status="running", action="sync:lab")
...
class TestAutoExtractConfigFiltering:
    ...
    async def test_configs_filtered_to_stop_node_names(self, test_db, test_user):
        ...
        host = make_host(test_db, host_id="agent-ext-filt")
        lab = make_lab(test_db, test_user.id, name="StopTest Lab", workspace_path="/tmp/stop-test", agent_id=host.id)
        job = make_job(test_db, lab.id, test_user.id, status="running", action="sync:lab")
    ...
    async def test_empty_node_name_or_content_skipped(self, test_db, test_user):
        ...
        host = make_host(test_db, host_id="agent-ext-empty")
        lab = make_lab(test_db, test_user.id, name="StopTest Lab", workspace_path="/tmp/stop-test", agent_id=host.id)
        job = make_job(test_db, lab.id, test_user.id, status="running", action="sync:lab")
    ...
    async def test_dedup_snapshot_not_counted(self, test_db, test_user):
        ...
        host = make_host(test_db, host_id="agent-ext-dedup")
        lab = make_lab(test_db, test_user.id, name="StopTest Lab", workspace_path="/tmp/stop-test", agent_id=host.id)
        job = make_job(test_db, lab.id, test_user.id, status="running", action="sync:lab")
...
class TestAutoExtractExceptionHandling:
    ...
    async def test_general_exception_caught_and_logged(self, test_db, test_user, caplog):
        ...
        host = make_host(test_db, host_id="agent-ext-exc")
        lab = make_lab(test_db, test_user.id, name="StopTest Lab", workspace_path="/tmp/stop-test", agent_id=host.id)
        job = make_job(test_db, lab.id, test_user.id, status="running", action="sync:lab")
    ...
    async def test_gather_exception_from_one_agent_does_not_block(self, test_db, test_user):
        ...
        host_a = make_host(test_db, host_id="agent-ext-ga", name="Agent A")
        host_b = make_host(test_db, host_id="agent-ext-gb", name="Agent B")
        lab = make_lab(test_db, test_user.id, name="StopTest Lab", workspace_path="/tmp/stop-test", agent_id=host_a.id)
        job = make_job(test_db, lab.id, test_user.id, status="running", action="sync:lab")
    ...
    async def test_no_configs_extracted_logs_message(self, test_db, test_user):
        ...
        host = make_host(test_db, host_id="agent-ext-noconf")
        lab = make_lab(test_db, test_user.id, name="StopTest Lab", workspace_path="/tmp/stop-test", agent_id=host.id)
        job = make_job(test_db, lab.id, test_user.id, status="running", action="sync:lab")
    ...
    async def test_extraction_builds_node_device_map(self, test_db, test_user):
        ...
        host = make_host(test_db, host_id="agent-ext-devmap")
        lab = make_lab(test_db, test_user.id, name="StopTest Lab", workspace_path="/tmp/stop-test", agent_id=host.id)
        job = make_job(test_db, lab.id, test_user.id, status="running", action="sync:lab")

        make_node(test_db, lab.id, gui_id="r1", display_name="R1", device="ceos")
        ns = make_node_state(test_db, lab.id, "r1", "R1", actual_state="running")
...
class TestStopNodesMultiAgent:
    ...
    async def test_three_agents_called_in_parallel(self, test_db, test_user):
        ...
        host_a = make_host(test_db, host_id="agent-ma-a", name="Agent A")
        host_b = make_host(test_db, host_id="agent-ma-b", name="Agent B")
        host_c = make_host(test_db, host_id="agent-ma-c", name="Agent C")
        lab = make_lab(test_db, test_user.id, name="StopTest Lab", workspace_path="/tmp/stop-test", agent_id=host_a.id)
        job = make_job(test_db, lab.id, test_user.id, status="running", action="sync:lab")

        ns1 = make_node_state(test_db, lab.id, "r1", "R1", desired_state="stopped", actual_state="running")
        ns2 = make_node_state(test_db, lab.id, "r2", "R2", desired_state="stopped", actual_state="running")
        ns3 = make_node_state(test_db, lab.id, "r3", "R3", desired_state="stopped", actual_state="running")
    ...
    async def test_not_found_on_default_agent_no_fallback(self, test_db, test_user):
        ...
        host = make_host(test_db, host_id="agent-ma-nofall")
        lab = make_lab(test_db, test_user.id, name="StopTest Lab", workspace_path="/tmp/stop-test", agent_id=host.id)
        job = make_job(test_db, lab.id, test_user.id, status="running", action="sync:lab")

        ns = make_node_state(test_db, lab.id, "r1", "R1", desired_state="stopped", actual_state="running")
    ...
    async def test_fallback_batch_with_multiple_nodes(self, test_db, test_user):
        ...
        host_default = make_host(test_db, host_id="agent-ma-def2", name="Default")
        host_other = make_host(test_db, host_id="agent-ma-oth2", name="Other")
        lab = make_lab(test_db, test_user.id, name="StopTest Lab", workspace_path="/tmp/stop-test", agent_id=host_default.id)
        job = make_job(test_db, lab.id, test_user.id, status="running", action="sync:lab")
...
class TestStopNodesMixedResults:
    ...
    async def test_mixed_success_failure_in_single_batch(self, test_db, test_user):
        ...
        host = make_host(test_db, host_id="agent-mix")
        lab = make_lab(test_db, test_user.id, name="StopTest Lab", workspace_path="/tmp/stop-test", agent_id=host.id)
        job = make_job(test_db, lab.id, test_user.id, status="running", action="sync:lab")
    ...
    async def test_no_placement_defaults_to_self_agent(self, test_db, test_user):
        ...
        host = make_host(test_db, host_id="agent-noplace")
        lab = make_lab(test_db, test_user.id, name="StopTest Lab", workspace_path="/tmp/stop-test", agent_id=host.id)
        job = make_job(test_db, lab.id, test_user.id, status="running", action="sync:lab")
    ...
    async def test_fallback_agent_unavailable_sets_transient(self, test_db, test_user):
        ...
        host_default = make_host(test_db, host_id="agent-fb-unav-d", name="Default")
        host_other = make_host(test_db, host_id="agent-fb-unav-o", name="Other")
        lab = make_lab(test_db, test_user.id, name="StopTest Lab", workspace_path="/tmp/stop-test", agent_id=host_default.id)
        job = make_job(test_db, lab.id, test_user.id, status="running", action="sync:lab")
    ...
    async def test_fallback_exception_sets_error_state(self, test_db, test_user):
        ...
        host_default = make_host(test_db, host_id="agent-fb-exc-d", name="Default")
        host_other = make_host(test_db, host_id="agent-fb-exc-o", name="Other")
        lab = make_lab(test_db, test_user.id, name="StopTest Lab", workspace_path="/tmp/stop-test", agent_id=host_default.id)
        job = make_job(test_db, lab.id, test_user.id, status="running", action="sync:lab")
...
class TestApplyStopResultDetailed:
    ...
    def test_success_clears_all_fields_and_broadcasts(self, test_db, test_user):
        ...
        host = make_host(test_db, host_id="agent-asr-1")
        lab = make_lab(test_db, test_user.id, name="StopTest Lab", workspace_path="/tmp/stop-test", agent_id=host.id)
        job = make_job(test_db, lab.id, test_user.id, status="running", action="sync:lab")
    ...
    def test_failure_defaults_to_stop_failed(self, test_db, test_user):
        ...
        host = make_host(test_db, host_id="agent-asr-2")
        lab = make_lab(test_db, test_user.id, name="StopTest Lab", workspace_path="/tmp/stop-test", agent_id=host.id)
        job = make_job(test_db, lab.id, test_user.id, status="running", action="sync:lab")
    ...
    def test_failure_preserves_custom_error(self, test_db, test_user):
        ...
        host = make_host(test_db, host_id="agent-asr-3")
        lab = make_lab(test_db, test_user.id, name="StopTest Lab", workspace_path="/tmp/stop-test", agent_id=host.id)
        job = make_job(test_db, lab.id, test_user.id, status="running", action="sync:lab")
    ...
    def test_success_tracks_old_state_in_log(self, test_db, test_user):
        ...
        host = make_host(test_db, host_id="agent-asr-4")
        lab = make_lab(test_db, test_user.id, name="StopTest Lab", workspace_path="/tmp/stop-test", agent_id=host.id)
        job = make_job(test_db, lab.id, test_user.id, status="running", action="sync:lab")
    ...
    def test_failure_clears_timestamps(self, test_db, test_user):
        ...
        host = make_host(test_db, host_id="agent-asr-5")
        lab = make_lab(test_db, test_user.id, name="StopTest Lab", workspace_path="/tmp/stop-test", agent_id=host.id)
        job = make_job(test_db, lab.id, test_user.id, status="running", action="sync:lab")
...
class TestConvergeStoppedDesiredErrorComprehensive:
    ...
    def test_reset_enforcement_called(self, test_db, test_user):
        ...
        host = make_host(test_db, host_id="agent-conv-1")
        lab = make_lab(test_db, test_user.id, name="StopTest Lab", workspace_path="/tmp/stop-test", agent_id=host.id)
        job = make_job(test_db, lab.id, test_user.id, status="running", action="sync:lab")

        ns = make_node_state(test_db, lab.id, "r1", "R1", desired_state="stopped", actual_state="error")
    ...
    def test_mixed_states_only_matching_normalized(self, test_db, test_user):
        ...
        host = make_host(test_db, host_id="agent-conv-2")
        lab = make_lab(test_db, test_user.id, name="StopTest Lab", workspace_path="/tmp/stop-test", agent_id=host.id)
        job = make_job(test_db, lab.id, test_user.id, status="running", action="sync:lab")

        ns_match = make_node_state(test_db, lab.id, "r1", "R1", desired_state="stopped", actual_state="error")
        ns_running = make_node_state(test_db, lab.id, "r2", "R2", desired_state="running", actual_state="error")
        ns_stopped = make_node_state(test_db, lab.id, "r3", "R3", desired_state="stopped", actual_state="stopped")
    ...
    def test_zero_matches_returns_zero(self, test_db, test_user):
        ...
        host = make_host(test_db, host_id="agent-conv-3")
        lab = make_lab(test_db, test_user.id, name="StopTest Lab", workspace_path="/tmp/stop-test", agent_id=host.id)
        job = make_job(test_db, lab.id, test_user.id, status="running", action="sync:lab")

        ns = make_node_state(test_db, lab.id, "r1", "R1", desired_state="running", actual_state="running")
    ...
    def test_is_ready_set_to_false(self, test_db, test_user):
        ...
        host = make_host(test_db, host_id="agent-conv-4")
        lab = make_lab(test_db, test_user.id, name="StopTest Lab", workspace_path="/tmp/stop-test", agent_id=host.id)
        job = make_job(test_db, lab.id, test_user.id, status="running", action="sync:lab")
    ...
    def test_clears_image_sync_and_timestamps(self, test_db, test_user):
        ...
        host = make_host(test_db, host_id="agent-conv-5")
        lab = make_lab(test_db, test_user.id, name="StopTest Lab", workspace_path="/tmp/stop-test", agent_id=host.id)
        job = make_job(test_db, lab.id, test_user.id, status="running", action="sync:lab")
    ...
    def test_broadcasts_stopped_for_each_normalized(self, test_db, test_user):
        ...
        host = make_host(test_db, host_id="agent-conv-6")
        lab = make_lab(test_db, test_user.id, name="StopTest Lab", workspace_path="/tmp/stop-test", agent_id=host.id)
        job = make_job(test_db, lab.id, test_user.id, status="running", action="sync:lab")

        ns1 = make_node_state(test_db, lab.id, "r1", "R1", desired_state="stopped", actual_state="error")
        ns2 = make_node_state(test_db, lab.id, "r2", "R2", desired_state="stopped", actual_state="error")
        ns3 = make_node_state(test_db, lab.id, "r3", "R3", desired_state="running", actual_state="error")
    ...
    def test_error_message_cleared(self, test_db, test_user):
        ...
        host = make_host(test_db, host_id="agent-conv-7")
        lab = make_lab(test_db, test_user.id, name="StopTest Lab", workspace_path="/tmp/stop-test", agent_id=host.id)
        job = make_job(test_db, lab.id, test_user.id, status="running", action="sync:lab")
    ...
    def test_empty_node_states_returns_zero(self, test_db, test_user):
        ...
        host = make_host(test_db, host_id="agent-conv-8")
        lab = make_lab(test_db, test_user.id, name="StopTest Lab", workspace_path="/tmp/stop-test", agent_id=host.id)
        job = make_job(test_db, lab.id, test_user.id, status="running", action="sync:lab")
