...
@pytest.mark.asyncio
async def test_verify_same_host_link_backfills_missing_host_ids(
    ...
    assert link_state.source_host_id == sample_host.id
    assert link_state.target_host_id == sample_host.id
...
@pytest.mark.asyncio
async def test_verify_cross_host_link_backfills_missing_host_ids(
    test_db, sample_lab, multiple_hosts, monkeypatch
) -> None:
    host_a, host_b = multiple_hosts[:2]
    test_db.add_all([
        models.Node(
    ...
    assert ok
    assert error is None
    assert link_state.source_host_id == host_a.id
    assert link_state.target_host_id == host_b.id
...
@pytest.mark.asyncio
async def test_verify_cross_host_link_does_not_match_by_interface_name(
    test_db,
    sample_lab,
    multiple_hosts,
    monkeypatch,
    ...
    async def fake_overlay_status(agent):
        return {
            "link_tunnels": [{
                "link_id": "placeholder-key",
                "interface_name": "vxlan-abcdef12",
                "vni": 40001,
                "local_vlan": 100 if agent.id == host_a.id else 200,
            }]
        }
    ...
    monkeypatch.setattr(
        "app.services.link_validator.agent_client.compute_vxlan_port_name",
        lambda *_args, **_kwargs: "vxlan-abcdef12",
    )
