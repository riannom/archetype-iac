...
def _make_node(db, lab_id, gui_id, name, device="linux", host_id=None, config_json=None, image=None):
    n = models.Node(
        lab_id=lab_id, gui_id=gui_id, display_name=name,
        container_name=name, device=device, host_id=host_id,
        config_json=config_json, image=image,
    )
    db.add(n)
    db.flush()
    return n
...
def _make_link(db, lab_id, src_node_id, src_iface, tgt_node_id, tgt_iface, config_json=None):
    link_name = f"{src_iface}-{tgt_iface}"  # Simplified
    lnk = models.Link(
...
class TestExportToGraph:
    ...
    def test_nodes_with_config_json(self, test_db: Session, sample_lab: models.Lab):
        ...
        _make_node(test_db, sample_lab.id, "n1", "R1", config_json=config)
        test_db.commit()
    ...
    def test_node_host_exported(self, test_db: Session, sample_lab: models.Lab):
        _make_host(test_db, "host-1", "Agent 1")
        _make_node(test_db, sample_lab.id, "n1", "R1", host_id="host-1")
        test_db.commit()
...
class TestMultihost:
    ...
    def test_single_host(self, test_db: Session, sample_lab: models.Lab):
        ...
        _make_node(test_db, sample_lab.id, "n2", "R2", host_id="h1")
        test_db.commit()
    ...
    def test_multihost_detected(self, test_db: Session, sample_lab: models.Lab):
        ...
        _make_node(test_db, sample_lab.id, "n1", "R1", host_id="h1")
        _make_node(test_db, sample_lab.id, "n2", "R2", host_id="h2")
        test_db.commit()
    ...
    def test_cross_host_links_detected(self, test_db: Session, sample_lab: models.Lab):
        _make_host(test_db, "h1")
        _make_host(test_db, "h2")
        n1 = _make_node(test_db, sample_lab.id, "n1", "R1", host_id="h1")
        n2 = _make_node(test_db, sample_lab.id, "n2", "R2", host_id="h2")
        _make_link(test_db, sample_lab.id, n1.id, "eth1", n2.id, "eth1")
        test_db.commit()
...
class TestImageMethods:
    ...
    def test_get_required_images_unique(self, mock_kind, mock_image, test_db: Session, sample_lab: models.Lab):
        _make_node(test_db, sample_lab.id, "n1", "R1", device="ceos")
        _make_node(test_db, sample_lab.id, "n2", "R2", device="ceos")
        test_db.commit()
    ...
    def test_get_required_images_skips_none(self, mock_kind, mock_image, test_db: Session, sample_lab: models.Lab):
        _make_node(test_db, sample_lab.id, "n1", "R1", device="unknown")
        test_db.commit()
...
class TestUpdateFromGraph:
    ...
    def test_deletes_missing_nodes(self, test_db: Session, sample_lab: models.Lab):
        _make_node(test_db, sample_lab.id, "old-gui", "OldNode")
        test_db.commit()
    ...
    def test_updates_existing_node(self, test_db: Session, sample_lab: models.Lab):
        _make_node(test_db, sample_lab.id, "n1", "OldName", device="linux")
        test_db.commit()
...
class TestToTopologyYamlForHost:
    ...
    def test_filters_to_single_host(self, test_db: Session, sample_lab: models.Lab):
        _make_host(test_db, "h1")
        _make_host(test_db, "h2")
        _make_node(test_db, sample_lab.id, "n1", "R1", host_id="h1", device="linux")
        _make_node(test_db, sample_lab.id, "n2", "R2", host_id="h2", device="linux")
        test_db.commit()
