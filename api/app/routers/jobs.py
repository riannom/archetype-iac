    ...
    def _sync_create_job():
        with get_session() as database:
            ...
            for ns in node_states:
                ...
                # explicitly set desired_state=STOPPED, regardless of current
                # actual_state (covers transitional states like "stopping").
                # First deploy (actual=undeployed) still starts all nodes.
                if (ns.desired_state == NodeDesiredState.STOPPED.value
                        and ns.actual_state != NodeActualState.UNDEPLOYED.value):
                    continue
