...
def _print_coverage(cfg: Config) -> int:
    checks = {
        ...
        "jobs_started": f'sum(increase(archetype_jobs_total{{job=~"archetype-(api|worker)",status="started"}}[{cfg.window}]))',
        "job_duration_samples": f'sum(increase(archetype_job_duration_seconds_count{{job=~"archetype-(api|worker)"}}[{cfg.window}]))',
        "job_queue_wait_samples": f'sum(increase(archetype_job_queue_wait_seconds_count{{job=~"archetype-(api|worker)"}}[{cfg.window}]))',
        "job_failure_reason_samples": f'sum(increase(archetype_job_failures_total{{job=~"archetype-(api|worker)"}}[{cfg.window}]))',
