...
export function useJellyfishDrift(
  ...
  const drawParticle = (ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement, particle: Particle, time: number) => {
  ...
  const drawJellyfish = (ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement, jelly: Jellyfish, time: number) => {
    // Update position
  ...
  useCanvasAnimation(canvasRef, active, {
    ...
    onFrame: (ctx, canvas) => {
      ...
      // Draw particles
      particlesRef.current.forEach((particle) => drawParticle(ctx, canvas, particle, time));

      // Draw jellyfish (sort by y for depth)
      jellyfishRef.current
        .sort((a, b) => a.y - b.y)
        .forEach((jelly) => drawJellyfish(ctx, canvas, jelly, time));
    },
  }, [darkMode, opacity]);
}
