...
export function useLavenderFields(
  ...
  const drawHills = (ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement) => {
  ...
  const drawLavender = (ctx: CanvasRenderingContext2D, time: number) => {
  ...
  const drawButterflies = (ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement, time: number) => {
  ...
  useCanvasAnimation(canvasRef, active, {
    ...
    onFrame: (ctx, canvas) => {
      ...
      drawHills(ctx, canvas);
      drawLavender(ctx, time);
      drawButterflies(ctx, canvas, time);
    },
  }, [darkMode, opacity]);
}
