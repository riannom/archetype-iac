...
export function useTrainStation(
  ...
  function createTrain(canvas: HTMLCanvasElement, forcePlatform?: number): Train {
  ...
  function createPassenger(canvas: HTMLCanvasElement, platformY: number): Passenger {
  ...
  function drawPlatform(ctx: CanvasRenderingContext2D, platform: Platform, opacityMult: number) {
  ...
  function drawTrain(ctx: CanvasRenderingContext2D, _canvas: HTMLCanvasElement, train: Train, opacityMult: number) {
  ...
  function drawPassenger(ctx: CanvasRenderingContext2D, passenger: Passenger, opacityMult: number) {
  ...
  useCanvasAnimation(canvasRef, active, {
    ...
    onFrame: (ctx, canvas) => {
      ...
      // Draw platforms
      platformsRef.current.forEach(platform => drawPlatform(ctx, platform, opacityMultiplier));
      ...
      trainsRef.current.forEach((train, i) => {
        ...
        drawTrain(ctx, canvas, train, opacityMultiplier);
      });
      ...
      passengersRef.current.forEach((passenger, i) => {
        ...
        drawPassenger(ctx, passenger, opacityMultiplier);
      });
    },
  }, [darkMode, opacity]);
}
