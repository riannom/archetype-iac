...
export function useIceCrystals(
  ...
  const createCrystal = (canvasWidth: number, canvasHeight: number): Crystal => {
  ...
  const drawCrystal = (ctx: CanvasRenderingContext2D, crystal: Crystal, opacityMult: number) => {
    ctx.save();
  ...
  const drawFrostParticle = (ctx: CanvasRenderingContext2D, particle: FrostParticle, opacityMult: number, time: number) => {
  ...
  const drawFrostEdge = (ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement, opacityMult: number, time: number) => {
  ...
  useCanvasAnimation(canvasRef, active, {
    ...
    onFrame: (ctx, canvas) => {
      ...
      // Draw frost edges
      drawFrostEdge(ctx, canvas, opacityMult, time);
      ...
      particlesRef.current.forEach((particle) => {
        ...
        drawFrostParticle(ctx, particle, opacityMult, time);
      });

      // Spawn new crystals periodically
      if (crystalsRef.current.length < 8 && Math.random() < 0.01) {
        crystalsRef.current.push(createCrystal(canvas.width, canvas.height));
      }
      ...
      crystalsRef.current = crystalsRef.current.filter((crystal) => {
        ...
        drawCrystal(ctx, crystal, opacityMult);
        return true;
      });
    },
  }, [darkMode, opacity]);
