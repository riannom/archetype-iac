...
export function useBunnyMeadow(
  ...
  const drawGrass = (ctx: CanvasRenderingContext2D, grass: Grass) => {
    const sway = Math.sin(state.timeRef * 0.002 + grass.swayPhase) * 3;
  ...
  const drawFlower = (ctx: CanvasRenderingContext2D, flower: Flower) => {
    const sway = Math.sin(state.timeRef * 0.002 + flower.swayPhase) * 2;
  ...
  const drawButterfly = (ctx: CanvasRenderingContext2D, bf: Butterfly) => {
    const wingAngle = Math.sin(state.timeRef * 0.1 + bf.wingPhase) * 0.5;
  ...
  const drawBunny = (ctx: CanvasRenderingContext2D, bunny: Bunny) => {
    ...
    const earWave = Math.sin(state.timeRef * 0.02 + bunny.earPhase) * 3;
    const noseWiggle = Math.sin(state.timeRef * 0.15 + bunny.noseWiggle) * 2;
  ...
  const updateButterfly = (bf: Butterfly, width: number, height: number) => {
    ...
    if (dist < 15) {
      ...
      bf.x += (dx / dist) * 1.2;
      bf.y += (dy / dist) * 1.2 + Math.sin(state.timeRef * 0.05) * 0.5;
    }
  };
  ...
  useCanvasAnimation(
    ...
    active,
    {
      ...
      onFrame: (ctx, canvas) => {
        ...
        // Draw grass
        state.grassPatches.forEach(grass => drawGrass(ctx, grass));

        // Draw flowers
        state.flowers.forEach(flower => drawFlower(ctx, flower));
