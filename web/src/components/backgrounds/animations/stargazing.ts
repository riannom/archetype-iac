...
export function useStargazing(
  ...
  function isInSideZone(x: number, width: number): boolean {
  ...
  function generateCloudPuffs(baseSize: number): CloudPuff[] {
  ...
  function initializeScene(canvas: HTMLCanvasElement) {
    ...
    const constellationList: Constellation[] = [];
    ...
    constellationsRef.current = constellationList;
    ...
    firefliesRef.current = fireflyList;
  ...
  function drawSky(ctx: CanvasRenderingContext2D, width: number, height: number) {
    ...
    if (darkMode) {
      ...
      skyGrad.addColorStop(0.4, '#0a1525');
      skyGrad.addColorStop(0.7, '#1a2535');
      skyGrad.addColorStop(1, '#2a3545');
    } else {
      // Twilight mode for light theme
      skyGrad.addColorStop(0, '#1a1a3a');
      skyGrad.addColorStop(0.3, '#2a2a5a');
      skyGrad.addColorStop(0.6, '#3a3a7a');
  ...
  function drawConstellation(ctx: CanvasRenderingContext2D, constellation: Constellation) {
    ...
    constellation.stars.forEach(star => {
      const twinkle = Math.sin(timeRef.current * 0.015 + star.x) * 0.2 + 0.8;
  ...
  function drawShootingStar(ctx: CanvasRenderingContext2D, ss: ShootingStar) {
  ...
  function drawTree(ctx: CanvasRenderingContext2D, tree: Tree, height: number) {
    const baseY = height;
  ...
  function drawFirefly(ctx: CanvasRenderingContext2D, ff: Firefly) {
    const glow = Math.sin(timeRef.current * 0.03 + ff.glowPhase) * 0.5 + 0.5;
  ...
  function updateFirefly(ff: Firefly, width: number, height: number) {
    ...
    if (dist < 10) {
      ...
      ff.y += (dy / dist) * 0.15 + Math.sin(timeRef.current * 0.015 + ff.glowPhase) * 0.08;
    }
  }
