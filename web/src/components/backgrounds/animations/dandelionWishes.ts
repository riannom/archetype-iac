...
export function useDandelionWishes(
  ...
  function createSeed(width: number, _height: number, fromDandelion?: { x: number; y: number }): DandelionSeed {
  ...
  function drawDandelion(ctx: CanvasRenderingContext2D, dandelion: Dandelion, time: number) {
  ...
  function drawSeed(ctx: CanvasRenderingContext2D, seed: DandelionSeed, time: number) {
    // Update position
  ...
  useCanvasAnimation(
    ...
    active,
    {
      ...
      onFrame: (ctx, canvas) => {
        ...
        // Draw dandelions
        dandelionsRef.current.forEach((d) => drawDandelion(ctx, d, time));

        // Draw and update seeds
        seedsRef.current.forEach((seed, index) => {
          drawSeed(ctx, seed, time);
