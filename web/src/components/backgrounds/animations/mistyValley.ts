...
export function useMistyValley(
  ...
  const drawMountainLayer = (ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement, layer: MountainLayer, time: number) => {
  ...
  const drawMist = (ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement, mist: MistCloud) => {
  ...
  const drawBird = (ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement, bird: Bird) => {
    bird.x += bird.speed;
    bird.wingPhase += 0.1;
  ...
  useCanvasAnimation(canvasRef, active, {
    ...
    onFrame: (ctx, canvas) => {
      ...
      mountainsRef.current.forEach((layer) => drawMountainLayer(ctx, canvas, layer, time));

      // Draw mist
      mistRef.current.forEach((mist) => drawMist(ctx, canvas, mist));

      // Draw birds
      birdsRef.current.forEach((bird) => drawBird(ctx, canvas, bird));
    },
  }, [darkMode, opacity]);
}
