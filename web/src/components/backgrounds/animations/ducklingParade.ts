...
export function useDucklingParade(
  ...
  function drawGrass(ctx: CanvasRenderingContext2D, grass: Grass) {
  ...
  function drawDuck(ctx: CanvasRenderingContext2D, duck: Duck, direction: number) {
  ...
  function updateDucks(family: DuckFamily, width: number, canvasHeight: number) {
    ...
    if ((family.direction === 1 && leader.x > width + 100) ||
      ...
      family.pathY = getRandomSideY(canvasHeight);
  ...
  function updateButterfly(bf: Butterfly, width: number, height: number) {
    ...
    active,
    {
      ...
      onFrame: (ctx, canvas) => {
        ...
        families.forEach(family => {
          updateDucks(family, width, height);
