...
export function useHashStorm(
  ...
  const drawChar = (ctx: CanvasRenderingContext2D, hashChar: HashChar, opacityMult: number) => {
  ...
  const drawCore = (ctx: CanvasRenderingContext2D, opacityMult: number, time: number) => {
  ...
  useCanvasAnimation(canvasRef, active, {
    ...
    onFrame: (ctx, canvas) => {
      ...
      charsRef.current = charsRef.current.filter((hashChar) => {
        ...
        drawChar(ctx, hashChar, opacityMult);
        return true;
      });

      // Draw core on top
      drawCore(ctx, opacityMult, time);
    },
  }, [darkMode, opacity]);
}
