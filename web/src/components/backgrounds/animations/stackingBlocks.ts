  ...
  useCanvasAnimation(canvasRef, darkMode, opacity, active, {
    ...
    let lastBlockTime = 0;
      ...
      blocksRef.current = blocksRef.current.filter((block) => {
        ...
        drawBlock(ctx, block, darkMode, opacityMultiplier, time);
        return true;
      });
