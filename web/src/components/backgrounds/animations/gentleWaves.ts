...
export function useGentleWaves(
  ...
  function initializeElements(canvas: HTMLCanvasElement) {
  ...
  function drawWaveLayer(ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement, wave: WaveLayer, time: number) {
    ctx.beginPath();
  ...
  function drawBubble(ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement, bubble: Bubble) {
  ...
  useCanvasAnimation(
    ...
    active,
    {
      ...
      onFrame: (ctx, canvas) => {
        ...
        wavesRef.current.forEach((wave) => drawWaveLayer(ctx, canvas, wave, time));

        // Draw bubbles
        bubblesRef.current.forEach((bubble) => drawBubble(ctx, canvas, bubble));

        // Draw sparkles
        sparklesRef.current.forEach((sparkle) => drawSparkle(ctx, sparkle));
      },
    },
    [darkMode, opacity]
  );
