...
export function useAutumnWind(
  ...
  function createLeaf(canvas: HTMLCanvasElement, randomPos = false): Leaf {
  ...
  function createGust(canvas: HTMLCanvasElement): WindGust {
  ...
  function drawMapleLeaf(ctx: CanvasRenderingContext2D, size: number, curl: number) {
    const s = size * 0.5;
  ...
  function drawOakLeaf(ctx: CanvasRenderingContext2D, size: number, _curl: number) {
    const s = size * 0.5;
  ...
  function drawGinkgoLeaf(ctx: CanvasRenderingContext2D, size: number, _curl: number) {
    const s = size * 0.5;
  ...
  function drawElmLeaf(ctx: CanvasRenderingContext2D, size: number, _curl: number) {
    const s = size * 0.5;
  ...
  function drawLeaf(ctx: CanvasRenderingContext2D, leaf: Leaf, opacityMult: number) {
    ctx.save();
  ...
  function drawGust(ctx: CanvasRenderingContext2D, gust: WindGust, opacityMult: number) {
    ...
    active,
    {
      onInit: (_ctx, canvas) => {
        ...
        for (let i = 0; i < leafCount; i++) {
          leavesRef.current.push(createLeaf(canvas, true));
        }
      },
      onFrame: (ctx, canvas) => {
        timeRef.current += 16;
        ...
        if (Math.random() < 0.008) {
          gustsRef.current.push(createGust(canvas));
        }
        ...
        leavesRef.current.forEach((leaf, index) => {
          ...
          if (leaf.x > canvas.width + 50 || leaf.y > canvas.height + 50) {
            leavesRef.current[index] = createLeaf(canvas, false);
          }

          drawLeaf(ctx, leaf, opacityMult);
        });
      },
    },
    [darkMode, opacity]
