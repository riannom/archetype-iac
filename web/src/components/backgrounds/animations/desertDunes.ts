...
export function useDesertDunes(
  ...
  function getRandomSidePosition(width: number): number {
    if (Math.random() < 0.7) {
  ...
  function generateBushClusters(size: number): { x: number; y: number; size: number }[] {
    const clusters = [];
    for (let i = 0; i < 5; i++) {
      clusters.push({
  ...
  function generateGrassHeights(size: number): number[] {
    const heights = [];
    for (let i = 0; i < 8; i++) {
  ...
  function generateTumbleweedBranches(size: number): Tumbleweed['branches'] {
    const branches = [];
    for (let i = 0; i < 20; i++) {
  ...
  function drawSky(ctx: CanvasRenderingContext2D, width: number, height: number) {
    ...
    if (darkMode) {
      ...
      sceneRef.current.stars.forEach(star => {
        const twinkle = Math.sin(timeRef.current * 0.003 + star.twinklePhase) * 0.5 + 0.5;
  ...
  function drawDune(ctx: CanvasRenderingContext2D, dune: SandDune, _width: number, height: number) {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const ripplePhase = Math.sin(timeRef.current * 0.0003 + i) * 1.5;
      ctx.beginPath();
  ...
  function drawSandParticle(ctx: CanvasRenderingContext2D, particle: SandParticle) {
    const wobble = Math.sin(timeRef.current * particle.wobbleSpeed + particle.wobble) * 2;
  ...
  function drawCactus(ctx: CanvasRenderingContext2D, plant: DesertPlant) {
    const size = plant.size;
    const sway = Math.sin(timeRef.current * 0.0008 + plant.swayPhase) * 1.5;
    ctx.save();
  ...
  function drawSucculent(ctx: CanvasRenderingContext2D, plant: DesertPlant) {
    const size = plant.size;
  ...
  function drawDesertBush(ctx: CanvasRenderingContext2D, plant: DesertPlant) {
    const sway = Math.sin(timeRef.current * 0.001 + plant.swayPhase) * 2;
    const size = plant.size;
  ...
  function drawDesertGrass(ctx: CanvasRenderingContext2D, plant: DesertPlant) {
    ctx.save();
    ctx.translate(plant.x, plant.y);
    ...
    if (plant.grassHeights) {
      plant.grassHeights.forEach((grassHeight, i) => {
        const sway = Math.sin(timeRef.current * 0.001 + plant.swayPhase + i * 0.5) * 3;
        const grassX = (i - 4) * 4;
  ...
  function drawPlant(ctx: CanvasRenderingContext2D, plant: DesertPlant) {
    switch (plant.type) {
      case 'cactus':
  ...
  function drawTumbleweed(ctx: CanvasRenderingContext2D, tw: Tumbleweed) {
    const bounce = Math.abs(Math.sin(timeRef.current * 0.008 + tw.bouncePhase)) * 4;
  ...
  function drawHeatWave(ctx: CanvasRenderingContext2D, wave: HeatWave) {
    if (darkMode) return; // No heat waves at night

    const waveY = wave.y + Math.sin(timeRef.current * wave.speed + wave.phase) * 2;

    ctx.globalAlpha = 0.02;
    ctx.fillStyle = '#FFFFFF';
    ctx.beginPath();
    for (let x = 0; x < wave.width; x += 5) {
      const y = waveY + Math.sin((x / 20) + timeRef.current * 0.003 + wave.phase) * 4;
      if (x === 0) {
  ...
  useCanvasAnimation(
    ...
    active,
    {
      ...
      onFrame: (ctx, canvas) => {
        ...
        scene.heatWaves.forEach(wave => drawHeatWave(ctx, wave));

        // Sort dunes by layer and draw
        scene.dunes.sort((a, b) => a.layer - b.layer);
        scene.dunes.forEach(dune => drawDune(ctx, dune, width, height));

        // Draw sand particles (slower movement)
        scene.sandParticles.forEach(particle => {
        ...
        // Draw plants
        scene.plants.forEach(plant => drawPlant(ctx, plant));

        // Update and draw tumbleweeds (slower)
        scene.tumbleweeds.forEach(tw => {
          tw.x += tw.speed;
