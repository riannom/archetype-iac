  ...
  useCanvasAnimation(canvasRef, enabled, {
    onInit: (_ctx, canvas) => {
      ...
      for (let i = 0; i < segments; i++) {
        ...
        topBank.push(height * 0.22 + topOffset);
        bottomBank.push(height * 0.82 + bottomOffset);
      }
    ...
    onFrame: (ctx, canvas) => {
      ...
      const drawBoatShape = (s: number, color: string) => {
        // Hull
        ctx.beginPath();
        ctx.moveTo(-s * 0.5, 0);
      ...
      const drawPaperBoat = (boat: PaperBoat) => {
      ...
      causticsRef.current.forEach((caustic) => {
        ...
        if (caustic.y > topY && caustic.y < bottomY) {
          const gradient = ctx.createRadialGradient(
            ...
            caustic.x, caustic.y, caustic.size * pulse
          );
