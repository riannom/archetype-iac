...
export function useButterflyGarden(
  ...
  function drawButterfly(ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement, butterfly: Butterfly, time: number) {
  ...
  useCanvasAnimation(
    ...
    active,
    {
      ...
      onFrame: (ctx, canvas) => {
        ...
        sortedFlowers.forEach((flower) => drawFlower(ctx, flower, time));

        // Draw butterflies
        butterfliesRef.current.forEach((butterfly) => drawButterfly(ctx, canvas, butterfly, time));
      },
    },
    [darkMode, opacity]
  );
