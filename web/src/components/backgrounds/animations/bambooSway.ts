...
export function useBambooSway(
  ...
  function createLeaf(segment: number, side: number): BambooLeaf {
    return {
      segment,
      side,
  ...
  function createBranch(segment: number, side: number): BambooBranch {
  ...
  function drawBamboo(ctx: CanvasRenderingContext2D, stalk: BambooStalk) {
    ...
    for (let i = 0; i < stalk.segments; i++) {
      ...
      stalk.branches.forEach((branch) => {
        if (branch.segment === i) {
          ...
          branch.leaves.forEach((leaf) => {
            ...
            drawLeafShape(ctx, leafX, leafY, leaf, branchSway, colors, stalk.opacity, opacityMultiplier);
          });
      ...
      stalk.leaves.forEach((leaf) => {
        if (leaf.segment === i) {
          drawLeafShape(ctx, segX, segY, leaf, sway, colors, stalk.opacity, opacityMultiplier);
        }
      });
  ...
  useCanvasAnimation(
    ...
    active,
    {
      ...
      onFrame: (ctx, canvas) => {
        ...
        stalksRef.current.forEach((stalk) => drawBamboo(ctx, stalk));
      },
    },
    [darkMode, opacity]
  );
