...
export function useFireworks(
  ...
  function initializeSparkles(canvas: HTMLCanvasElement) {
  ...
  function launchRocket(canvas: HTMLCanvasElement) {
  ...
  function drawRocket(ctx: CanvasRenderingContext2D, rocket: Rocket) {
    // Update trail
    ...
    active,
    {
      ...
      onFrame: (ctx, canvas) => {
        ...
        if (time - lastLaunchRef.current > 800 + Math.random() * 1500) {
          ...
          if (Math.random() > 0.6) {
            setTimeout(() => launchRocket(canvas), 100 + Math.random() * 200);
          }
