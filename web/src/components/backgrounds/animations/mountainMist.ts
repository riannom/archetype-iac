...
export function useMountainMist(
  ...
  useCanvasAnimation(canvasRef, active, {
    onInit: (_ctx, canvas) => {
      ...
      const getRandomSidePosition = (w: number): number => {
        if (Math.random() < 0.7) {
      ...
      mistLayersRef.current = mistLayers;
    ...
    onFrame: (ctx, canvas) => {
      ...
      const waterReflection = waterReflectionRef.current;
      timeRef.current++;
      ...
      // Draw mountains helper
      const drawMountain = (mountain: Mountain) => {
      ...
      mistLayers.forEach(mist => {
        const mistOffset = Math.sin(timeRef.current * 0.001 + mist.offset) * 50;
        ...
        for (let x = 0; x <= width; x += 20) {
          const waveY = mist.y +
            Math.sin((x + mistOffset) * 0.01 + timeRef.current * mist.speed * 0.01) * mist.height * 0.5;
          ctx.lineTo(x, waveY);
        }
      ...
      if (waterReflection) {
        const waterGradient = ctx.createLinearGradient(0, waterReflection.y, 0, height);
        if (darkMode) {
        ...
        ctx.fillRect(0, waterReflection.y, width, height - waterReflection.y);
        ...
        for (let i = 0; i < 8; i++) {
          const rippleY = waterReflection.y + 10 + i * 15;
          const phase = timeRef.current * 0.005 + i * 0.5;
        ...
        ctx.globalAlpha = 0.2;
        ctx.save();
        ctx.translate(0, waterReflection.y * 2);
        ctx.scale(1, -0.3);
      ...
      birds.forEach(bird => {
        ...
        const wingAngle = Math.sin(timeRef.current * 0.15 + bird.wingPhase) * 0.5;
