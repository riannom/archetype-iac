...
describe("Canvas", () => {
  ...
  describe("Node Rendering", () => {
    ...
    it("shows blue sync indicator even before runtime status is set", () => {
      const node = createDeviceNode({ id: "node-1", name: "SyncNoRuntimeNode" });
      const nodeStates = {
        "node-1": {
          id: "state-1",
          node_id: "node-1",
          node_name: "SyncNoRuntimeNode",
          image_sync_status: "syncing",
          image_sync_message: "Syncing image to agent-01...",
        },
      };
  ...
  describe("X-Ray View", () => {
    ...
    it("does not show X-Ray toggle with only one agent", () => {
      renderWithTheme(
        <Canvas
          {...defaultProps}
          agents={[{ id: "agent-1", name: "Agent 1" }]}
          onToggleXRayView={vi.fn()}
        />
      );
      const icon = document.querySelector("button .fa-layer-group");
      expect(icon).not.toBeInTheDocument();
    });

    it("shows X-Ray toggle when multiple agents exist", () => {
      renderWithTheme(
        <Canvas
          {...defaultProps}
          agents={multiAgents}
    ...
    it("renders host zones when showXRayView is true with multi-host nodes", () => {
      const nodes = [
        createDeviceNode({ id: "n1", name: "R1", x: 100, y: 100 }),
        createDeviceNode({ id: "n2", name: "R2", x: 300, y: 200 }),
      ];
      const nodeStates = {
        n1: { id: "s1", node_id: "n1", node_name: "R1", host_id: "agent-1", host_name: "Agent One" },
        n2: { id: "s2", node_id: "n2", node_name: "R2", host_id: "agent-2", host_name: "Agent Two" },
      };
      ...
      // Host zone labels should be rendered
      const svg = document.querySelector("svg")!;
      const zoneTexts = svg.querySelectorAll("text");
      const zoneLabels = Array.from(zoneTexts)
        .map((t) => t.textContent)
        .filter((t) => t?.includes("arch-ovs"));
      expect(zoneLabels.length).toBe(2);
    });

    it("does not render host zones when showXRayView is false", () => {
      const nodes = [
        createDeviceNode({ id: "n1", name: "R1" }),
      ];
      const nodeStates = {
        n1: { id: "s1", node_id: "n1", node_name: "R1", host_id: "agent-1", host_name: "Agent One" },
      };
      ...
      const svg = document.querySelector("svg")!;
      const zoneLabels = Array.from(svg.querySelectorAll("text"))
        .filter((t) => t.textContent?.includes("arch-ovs"));
      expect(zoneLabels.length).toBe(0);
    });

    it("renders cross-host link with VNI badge in X-Ray mode", () => {
      const nodes = [
        createDeviceNode({ id: "n1", name: "R1", container_name: "R1", x: 100, y: 100 }),
        createDeviceNode({ id: "n2", name: "R2", container_name: "R2", x: 300, y: 200 }),
      ];
      const links: Link[] = [
        { id: "l1", source: "n1", target: "n2", sourceInterface: "eth1", targetInterface: "eth1" },
      ];
      const linkStatesMap = new Map([
        ["R1:eth1-R2:eth1", {
          link_name: "R1:eth1-R2:eth1",
          desired_state: "up" as const,
          actual_state: "up" as const,
          source_node: "R1",
          target_node: "R2",
          is_cross_host: true,
          vni: 50001,
      ...
      const svg = document.querySelector("svg")!;
      const allTexts = Array.from(svg.querySelectorAll("text")).map((t) => t.textContent);
      // Should show VNI badge
      expect(allTexts.some((t) => t?.includes("VNI 50001"))).toBe(true);
      // Should show VLAN tags
      expect(allTexts.some((t) => t === "v3001")).toBe(true);
      expect(allTexts.some((t) => t === "v3002")).toBe(true);
    });

    it("calls onToggleXRayView when X-Ray toggle is clicked", async () => {
      const user = userEvent.setup();
      const mockToggle = vi.fn();
      ...
      renderWithTheme(
        <Canvas
          {...defaultProps}
          agents={multiAgents}
          onToggleXRayView={mockToggle}
        />
      );

      const toggleButton = document.querySelector("button .fa-layer-group")!.closest("button")!;
      await user.click(toggleButton);
      expect(mockToggle).toHaveBeenCalled();
    });
  });
