...
describe('TaskLogPanel extended', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    localStorage.clear();
  });
  ...
  it('closes a console tab via the close button', () => {
    ...
    // The close button is a small X next to each tab name
    ...
    // stopPropagation prevents onSelectTab; onCloseConsoleTab fires
  ...
  it('triggers horizontal reorder within tab strip', () => {
    ...
    // Horizontal movement within the strip
    fireEvent.mouseMove(document, { clientX: 350, clientY: 72 });
    fireEvent.mouseUp(document, { clientX: 350, clientY: 72 });

    // Either reorder was called, or the movement was insufficient
    // The behavior depends on the calculated target index
    expect(onUndockConsole).not.toHaveBeenCalled();
  });
  ...
  it('shows error count badge when there are error entries', () => {
    ...
    // Should show "2" as error count badge
